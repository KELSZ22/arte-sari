...
class ProductFormComponent extends Component {
  ...
  handleSubmit(event) {
    ...
    const addons = form.querySelectorAll('.addon-checkbox:checked');
    addons.forEach((addon, idx) => {
      const addonId = addon.dataset.variantId;
      if (!addonId) return;

      // `idx + 1` because 0 is the main product
      formData.append(`items[${idx + 1}][id]`, addonId);
      formData.append(`items[${idx + 1}][quantity]`, 1);
    });
